...
export default function AuthModal() {
  ...
  useEffect(() => {
    if (showLoginModal) {
      setStage('launch');
      const t1 = setTimeout(() => setStage('warp'), 1200);
      const t2 = setTimeout(() => setStage('explosion'), 2000);
      const t3 = setTimeout(() => setStage('form'), 2200);
  ...
  return (
    // FIX: Using fixed inset-0 + flex to perfectly center on ALL devices
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black px-4 overflow-hidden">
      <AnimatePresence mode='wait'>
        ...
        {stage === 'launch' && (
          ...
          >
             {/* Background */}
            <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1506318137071-a8bcbf6dd04c')] opacity-40 bg-cover animate-pulse pointer-events-none" />
            ...
            <motion.h2
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="mt-8 text-blue-400 font-bold uppercase text-xl tracking-widest relative z-10"
            >
              FairBasket Loading...
            </motion.h2>
          </motion.div>
        ...
        {stage === 'warp' && (
           <motion.div key="warp" className="absolute inset-0 bg-black flex items-center justify-center">
             <div className="text-blue-500 animate-pulse font-mono">ENGAGING HYPERDRIVE...</div>
           </motion.div>
        )}
        ...
        {stage === 'form' && (
          <motion.div
            ...
            className="relative z-50 w-full max-w-sm bg-[#111]/95 backdrop-blur-xl border border-blue-500/30 rounded-3xl p-6 shadow-2xl"
          >
            <div className="text-center mb-6">
              ...
              <p className="text-blue-200/60 text-xs">Authentication Required</p>
            </div>
            ...
            <div className="mt-6 space-y-3">
              <button 
                ...
                className="w-full bg-blue-600 text-white font-bold py-3 rounded-xl uppercase text-sm tracking-wider hover:bg-blue-500 transition shadow-lg shadow-blue-900/20"
              >
                Enter Store
              </button>
              {!isForceLogin && (
                <button onClick={skipLogin} className="w-full text-center text-gray-500 text-xs py-2 hover:text-white">Skip for now</button>
              )}
            </div>
          </motion.div>
        )}
